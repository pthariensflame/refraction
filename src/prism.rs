...
impl<S, T, A, B, V, W, LF: Prism<S, T, A, B>, LS: Prism<A, B, V, W> + ?Sized> Prism<S, T, V, W> for Compose<S, T, A, B, V, W, LF, LS> {
...
impl<S, T, A, B, G: Deref<Target=Fn(S) -> Result<A, T>>, H: Deref<Target=Fn(B) -> T>> Prism<S, T, A, B> for PrismFn<G, H> {
	fn try_get(&self, v: S) -> Result<A, T> {
		(self.fst())(v)
	}
