...
impl<S, T, A, B, V, W, LF: Iso<S, T, A, B>, LS: Iso<A, B, V, W> + ?Sized> Iso<S, T, V, W> for Compose<S, T, A, B, V, W, LF, LS> {}
...
impl<S, T, A, B, G: Deref<Target=Fn(S) -> A>, H: Deref<Target=Fn(B) -> T> + ?Sized> Lens<S, T, A, B> for IsoFn<G, H> {
	fn get(&self, v: S) -> A {
		(self.fst())(v)
	}
...
impl<S, T, A, B, G: Deref<Target=Fn(S) -> A>, H: Deref<Target=Fn(B) -> T>> Prism<S, T, A, B> for IsoFn<G, H> {
	fn try_get(&self, v: S) -> Result<A, T> {
		Ok((self.fst())(v))
	}
...
impl<S, T, A, B, G: Deref<Target=Fn(S) -> A>, H: Deref<Target=Fn(B) -> T>> Iso<S, T, A, B> for IsoFn<G, H> {}
